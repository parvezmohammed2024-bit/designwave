...
export default function CheckoutFlow() {
  ...
  const amountDue = designFinalized ? Math.ceil(total / 2) : DESIGN_CHARGE;
  ...
  const uploadFiles = async (list: FileList | File[]) => {
    for (const file of Array.from(list)) {
      ...
      setFiles((f) => [
        ...f,
        { name: file.name, size: file.size, status: "uploading", path },
      ]);
  ...
  const placeOrder = async () => {
    ...
    const items = lines.map((l) => {
      ...
      return {
        ...
        name: p?.name ?? l.slug,
        tierQty: l.tierQty,
        lamination: l.lamination ?? null,
        unitPrice: l.unitPrice,
        quantity: l.quantity,
        lineTotal: l.unitPrice * l.quantity,
      };
    });
    const { error } = await supabase.from("dw_orders").insert({
      id: orderId,
      name: info.name.trim(),
    ...
    setSubmitting(false);
    if (error) {
      setSubmitError("অর্ডার জমা দেওয়া যায়নি — আবার চেষ্টা করুন বা হোয়াটসঅ্যাপে যোগাযোগ করুন।");
      return;
    }
    setPlaced(true);
    clear();
  ...
  const orderSummaryText = () => {
    const itemLines = lines
      .map((l) => {
        const p = getProduct(l.slug);
        return `• ${p?.name} — ${toBanglaDigits(l.tierQty)} পিস × ${toBanglaDigits(l.quantity)}`;
      })
      .join("\n");
    return `আসসালামু আলাইকুম! আমার অর্ডার:\nঅর্ডার আইডি: ${orderId}\n${itemLines}\nমোট: ${formatTaka(total)}\nপ্রদেয়: ${formatTaka(amountDue)}`;
  };
            ...
            <section aria-label="আপনার তথ্য" className="space-y-5">
              ...
              {info.district === "চট্টগ্রাম" && (
                <fieldset>
                  ...
                  <div className="flex gap-2">
                    {[
                      { v: true, t: `সিটির ভেতরে · ${formatTaka(DELIVERY_INSIDE_CTG)}` },
                      { v: false, t: `সিটির বাইরে · ${formatTaka(DELIVERY_OUTSIDE_CTG)}` },
                    ].map((o) => (
              ...
              <div>
                ...
                <textarea id="co-address" rows={3} className={field}
                  placeholder="বাসা/হোল্ডিং, রোড, এলাকা, থানা"
                  value={info.address}
              ...
              <div>
                ...
                <textarea id="co-note" rows={2} className={field}
                  value={info.note}
                  onChange={(e) => setInfo({ ...info, note: e.target.value })}
                />
              </div>
              <button type="button"
                onClick={() => validateInfo() && setStep(1)}
          ...
          {step === 1 && (
            <section aria-label="ডিজাইন ফাইল" className="space-y-5">
              <p className="leading-bangla text-ink/70">
                ...
                <a href={waLink(`অর্ডার ${orderId} — ডিজাইন ফাইল পাঠাচ্ছি।`)}
                    ...
                    <li key={i} className="flex items-center gap-3 rounded-xl border border-ink/10 px-4 py-2.5 text-sm">
                      ...
                      {f.status === "uploading" && (
                        <span className="relative h-1.5 w-20 overflow-hidden rounded-full bg-ink/10">
                          <motion.span
                            className="absolute inset-y-0 w-1/3 rounded-full bg-wave-500"
                            animate={{ x: ["-100%", "300%"] }}
              ...
              <fieldset className="rounded-2xl border border-ink/10 p-4">
                ...
                {[
                  { v: false, t: "ডিজাইন Design Wave করবে", d: `ডিজাইন চার্জ ${formatTaka(DESIGN_CHARGE)} — অনুমোদনের পর বাকি পেমেন্ট` },
            ...
            <section aria-label="পেমেন্ট" className="space-y-5">
              <div className="rounded-2xl border border-ink/10 p-5">
                <div className="flex justify-between text-sm">
                  <span>সাবটোটাল</span><span>{formatTaka(subtotal)}</span>
                </div>
                ...
                <div className="mt-2 flex justify-between border-t border-ink/10 pt-2 font-bold">
                  <span>মোট</span><span>{formatTaka(total)}</span>
                </div>
              </div>

              <div className="rounded-2xl bg-brand-700 p-5 text-paper">
                <p className="text-sm text-paper/80">এখন প্রদেয় ({designFinalized ? "৫০% অ্যাডভান্স" : "ডিজাইন চার্জ"})</p>
                <p className="mt-1 text-3xl font-bold text-wave-300">{formatTaka(amountDue)}</p>
                <div className="mt-4 flex items-center justify-between gap-3 rounded-xl bg-paper/10 px-4 py-3">
                  <div>
                    ...
                    <p dir="ltr" className="text-lg font-bold tracking-wider">{PAYMENT_NUMBER_BN}</p>
                  </div>
                ...
                <ol className="mt-4 space-y-1 text-sm leading-bangla text-paper/85">
                  <li>১. ডিজাইন চার্জ {formatTaka(DESIGN_CHARGE)} (ডিজাইন লাগলে)</li>
          ...
          {step === 3 && placed && (
            <section aria-label="অর্ডার নিশ্চিত" className="text-center">
              ...
              <div className="mx-auto mt-6 max-w-sm rounded-2xl border border-ink/10 p-5 text-left text-sm">
                <div className="flex justify-between"><span>মোট</span><span className="font-semibold">{formatTaka(total)}</span></div>
                <div className="mt-1 flex justify-between"><span>এখন প্রদেয়</span><span className="font-semibold text-brand-700">{formatTaka(amountDue)}</span></div>
